...
type Props = { type: Type; message: string }
...
export const AlertMessage = ({ type, message }: Props) => (
    <View style={[styles.container, { backgroundColor: colors[type].bg }]}>
        <AppText variant="p" color={colors[type].text}>{message}</AppText>
    </View>
)

const styles = StyleSheet.create({
    container: { borderRadius: theme.radius.sm, padding: theme.spacing.md, marginBottom: theme.spacing.lg }
})