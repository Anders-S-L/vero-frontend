...
function AfdelingerTab({ token }: { token: string }) {
  const { departments, isLoading, error, addDepartment } =
...
function CategorySection({
  ...
  const { categories, isLoading, addCategory } = useCategoryViewModel(
    token,
    departmentId,
  );
...
function TransactionSection({
  ...
  const { transactions, isLoading, addTransaction } = useTransactionViewModel(
    token,
    categoryId,
  );
  const [showModal, setShowModal] = useState(false);
