...
const normalizeMember = (raw: any): TeamMember | null => {
    ...
    if (!id || !fullName || !email || !departmentId) return null
...
export const useTeamViewModel = (token: string) => {
    ...
    const fetchMembers = useCallback(async () => {
        ...
        try {
            const rawData = await getJson<any[]>('/profiles', token)
            const normalized = rawData.map(normalizeMember).filter(Boolean) as TeamMember[]
